...
ClientReturnCode SDKMinimalClient::Initialize()
{
	...
	if (!InitializeSocket()) {
		...
		// Do not return error, allow program to continue (only show warning)
	}
...
void SDKMinimalClient::OnRawSkeletonStreamCallback(const SkeletonStreamInfo* const p_RawSkeletonStreamInfo)
{
	if (s_Instance)
	{
		...
		// Send data to Python - REMOVED: will send combined data in Run loop
...
// ==================== Tracker callback function implementation ====================
...
void SDKMinimalClient::OnTrackerStreamCallback(const TrackerStreamInfo* const p_TrackerStreamInfo)
{
	if (s_Instance)
	{
		...
		// Send Tracker data to Python - REMOVED: will send combined data in Run loop
...
// ==================== Tracker data sending and JSON serialization ====================

std::string SDKMinimalClient::TrackerToJSON(const TrackerDataCollection* data) {
	if (!data || data->trackerData.empty()) {
		return "{}";
	}
	...
	for (size_t trackerIdx = 0; trackerIdx < data->trackerData.size(); trackerIdx++) {
		const auto& tracker = data->trackerData[trackerIdx];
		const auto& pos = tracker.position;
		const auto& rot = tracker.rotation;

		json << "{";
		json << "\"id\":" << trackerIdx << ",";
		json << "\"trackerId\":\"" << std::hex << tracker.trackerId.id << std::dec << "\",";
		json << "\"position\":[" << pos.x << "," << pos.y << "," << pos.z << "],";
		json << "\"rotation\":[" << rot.x << "," << rot.y << "," << rot.z << "," << rot.w << "],";
		json << "\"quality\":" << (tracker.quality != TrackingQuality::TrackingQuality_Untrackable ? "true" : "false");

		if (trackerIdx < data->trackerData.size() - 1) {
			json << "},";
		} else {
			json << "}";
		}
...
std::string SDKMinimalClient::CombinedToJSON(const ClientRawSkeletonCollection* skeletonData, const TrackerDataCollection* trackerData) {
	std::stringstream json;
	json << std::fixed << std::setprecision(6);
	...
	// Add skeleton data
	json << "\"skeletons\":";
	if (skeletonData && !skeletonData->skeletons.empty()) {
		json << "[";
		for (size_t skeletonIdx = 0; skeletonIdx < skeletonData->skeletons.size(); skeletonIdx++) {
			const auto& skeleton = skeletonData->skeletons[skeletonIdx];
			...
			if (skeletonIdx < skeletonData->skeletons.size() - 1) {
				json << "},";
			} else {
				json << "}";
			}
	...
	// Add Tracker data
	json << "\"trackers\":";
	if (trackerData && !trackerData->trackerData.empty()) {
		json << "[";
		for (size_t trackerIdx = 0; trackerIdx < trackerData->trackerData.size(); trackerIdx++) {
			const auto& tracker = trackerData->trackerData[trackerIdx];
			const auto& pos = tracker.position;
			const auto& rot = tracker.rotation;

			json << "{";
			json << "\"id\":" << trackerIdx << ",";
			json << "\"trackerId\":\"" << tracker.trackerId.id << "\",";
			json << "\"position\":[" << pos.x << "," << pos.y << "," << pos.z << "],";
			json << "\"rotation\":[" << rot.x << "," << rot.y << "," << rot.z << "," << rot.w << "],";
...
void SDKMinimalClient::SendTrackerData(const TrackerDataCollection* data) {
	if (m_ClientSocket == INVALID_SOCKET_VAL || !m_SocketInitialized) {
		return; // Socket not initialized, do not send data
	}
...
// ==================== Modified RegisterAllCallbacks to register Tracker callback ====================
...
ClientReturnCode SDKMinimalClient::RegisterAllCallbacks()
{
	...
	// Register the callback to receive Tracker data
	// Added: Register Tracker data callback
	...
	{
		ClientLog::warn("Failed to register callback function for tracker data. The value returned was {}.", (int32_t)t_RegisterTrackerCallbackResult);
		// Do not return error, allow program to continue (Tracker data is optional)
	}

	return ClientReturnCode::ClientReturnCode_Success;
}

// ==================== Modified Run function to handle Tracker data ====================
...
void SDKMinimalClient::Run()
{
	...
	while (m_Running)
	{
		...
		if (hasNewTrackerData && m_TrackerData != nullptr && m_TrackerData->trackerData.size() != 0)
		{
			...
			for (size_t i = 0; i < m_TrackerData->trackerData.size(); i++) {
				const auto& tracker = m_TrackerData->trackerData[i];
				ClientLog::print("  Tracker {}: trackerId={}, position=[{}, {}, {}], quality={}",
					i, tracker.trackerId.id, tracker.position.x, tracker.position.y,tracker.position.z, static_cast<int>(tracker.quality));
			}
		}
